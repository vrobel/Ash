package net.richardlord.ash.core
{
	 ...
	 */
	public class ComponentExcludedMatchingFamily implements Family
	{
		private var nodes : NodeList;
		private var entities : Dictionary;
		private var nodeClass : Class;
		private var components : Dictionary;
		private var excludedComponents : Dictionary;
		private var nodePool : NodePool;
		private var game : Game;

		public function ComponentExcludedMatchingFamily( nodeClass : Class, game : Game )
		{
		...
		private function init() : void
		{
			...
			if( nodeClass.hasOwnProperty('excluded'))
			{
				var excluded : Array = nodeClass['excluded'] as Array;
				if ( excluded && excluded.length > 0 )
				{
					excludedComponents = new Dictionary();
					for (var i:int = 0; i < excluded.length; i++) {
						componentClass = excluded[i];
						if (componentClass)
						{
							excludedComponents[componentClass] = true;
						}
					}
				}
			}
		...
		public function componentRemovedFromEntity( entity : Entity, componentClass : Class ) : void
		{
			...
			else if ( excludedComponents && excludedComponents[componentClass] )
			{
		...
		private function addIfMatch( entity : Entity ) : void
		{
			if( !entities[entity] )
			{
				...
				if (excludedComponents) {
					for (componentClass in excludedComponents) {
						if (entity.has(componentClass)) {
							return;
						}
					}
